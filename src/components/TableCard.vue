<template>
  <div class="page-table">
    ...
    <div v-else-if="tableError" class="text-16 no-data spaced">
          ...
          <tbody v-for="(user, i) in usersData" :key="`user${i}`">
            ...
            >
              <td class="table-check">
                <check-button
                  :id="`check${i}`"
                  @option-checked="checkOption(i, user)"
                />
                <div
                  class="icon"
                  :class="{ toggle: allVisible[i] }"
              ...
              <td>
                ...
                <div class="text-12 td-bold td-secondary">
                  Last Login: {{ formattedDate(user.lastLogin) }}
                </div>
              </td>
              <td>
                ...
                <div v-if="user.paidOn" class="text-12 td-bold td-primary">
                  Paid on {{ formattedDate(user.paidOn) }}
                </div>
                <div v-else class="text-12 td-bold td-primary">
                  <span class="span">{{ user.paymentStatus }} </span> as at
            ...
            <table-row v-if="allVisible[i]" class="activity-table-wrapper">
              <td class="activity-td" colspan="6">
                <div class="activity-table">
                  <table-wrapper
                    v-if="currentUserWithActivities.activities.length > 0"
                    tableWidth="700px"
                  >
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      <table-row
                        v-for="(
                          activity, i
                        ) in currentUserWithActivities.activities"
                        :key="`activity${i}`"
                      >
                        <td class="first-cell text-14 td-secondary spaced">
                          {{ formattedDate(activity.date) }}
                        </td>
                        <td class="middle-cell text-14 td-primary spaced">
                          {{ activity.userActivity }}
                        </td>
                        <td class="text-14 td-primary spaced">
                          {{ activity.details }}
                        </td>
                      </table-row>
                    </tbody>
                  </table-wrapper>
                  <div v-else class="text-16 no-data spaced">
                    No Records found.
                  </div>
                </div>
              </td>
...
export default { 
  ...
  watch: {
    currentCheckIndex(newVal, oldVal) {
      ...
      if (newVal == null) {
        //checks for index with null values so as to skip this step of removing class
  ...
  methods: {
    ...
    checkOption(index, user) {
      ...
      if (this.currentCheckIndex == null) {
        ...
        document
          .getElementById(`check${this.currentCheckIndex}`)
